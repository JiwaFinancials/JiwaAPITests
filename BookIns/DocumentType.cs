...
namespace JiwaAPITests.BookIns
{
    public class DocumentType : JiwaAPITest
    {
        #region "{Main}"
        [Test]
        public async Task BookInDocumentType_CRUD()
        {
            // Create a document type
            LandedCostBookInDocumentTypePOSTRequest documentTypeCreateReq = new LandedCostBookInDocumentTypePOSTRequest()
            {
                Description = RandomString(12),
                DefaultType = false
            };

            BookInDocumentTypeDto documentTypeCreateRes = await Client.PostAsync(documentTypeCreateReq);
            ...
            LandedCostBookInDocumentTypeGETRequest documentTypeGetReq = new LandedCostBookInDocumentTypeGETRequest()
            {
                DocumentTypeID = documentTypeCreateRes.DocumentTypeID
            };

            BookInDocumentTypeDto documentTypeGetRes = await Client.GetAsync(documentTypeGetReq);
            Assert.That(LastHttpStatusCode, Is.EqualTo(System.Net.HttpStatusCode.OK));
            ...
            LandedCostBookInDocumentTypePATCHRequest documentTypePatchReq = new LandedCostBookInDocumentTypePATCHRequest()
            {
                DocumentTypeID = documentTypeCreateRes.DocumentTypeID,
                Description = "Updated " + RandomString(10)
            };

            BookInDocumentTypeDto documentTypePatchRes = await Client.PatchAsync(documentTypePatchReq);
            Assert.That(LastHttpStatusCode, Is.EqualTo(System.Net.HttpStatusCode.OK));
            ...
            // Read the updated document type and confirm the description was changed
            ...
            LandedCostBookInDocumentTypeDELETERequest documentTypeDeleteReq = new LandedCostBookInDocumentTypeDELETERequest()
            {
                DocumentTypeID = documentTypeCreateRes.DocumentTypeID
            };
            ...
            {
                BookInDocumentTypeDto getDeletedRes = await Client.GetAsync(documentTypeGetReq);
            });
